...
ALTER TABLE properties
  ...
  GENERATED ALWAYS AS (
    to_tsvector('english'::regconfig,
      ...
      coalesce(cooling_type,   '') || ' ' ||
      coalesce(laundry_type,   '') || ' ' ||
      immutable_array_to_text(coalesce(amenities, '{}'::text[]), ' ') || ' ' ||
      immutable_array_to_text(coalesce(appliances, '{}'::text[]), ' ') || ' ' ||
      immutable_array_to_text(coalesce(utilities_included, '{}'::text[]), ' ') || ' ' ||
      immutable_array_to_text(coalesce(flooring, '{}'::text[]), ' ')
    )
  ) STORED;
...
ALTER TABLE properties
  ADD COLUMN IF NOT EXISTS total_bathrooms FLOAT,
  ADD COLUMN IF NOT EXISTS has_basement BOOLEAN DEFAULT false,
  ADD COLUMN IF NOT EXISTS has_central_air BOOLEAN DEFAULT false;
