...
# ── Enrichment-input signature constants ───────────────────────────────────────
#
# These are the fields that come from scraping / human editing, NOT from AI
# enrichment. A change in any of them means the AI output may be stale and
# enrichment should re-run. Fields generated by AI (description, title,
# amenities, appliances) are deliberately excluded so that saving an AI-generated
# value doesn't create a circular trigger.

CORE_SCRAPED_FIELDS = frozenset([
    "address", "city", "state", "zip",
    "bedrooms", "bathrooms", "monthly_rent", "property_type",
    "square_footage", "year_built", "available_date",
    "parking", "utilities_included", "pets_allowed",
...
# ── Enrichment signature helpers (Phase 3C) ────────────────────────────────────

def _compute_input_sig(prop) -> str:
    """
    MD5 fingerprint of the core scraped fields. Changes when scraping or human
    editing alters the inputs that drive AI enrichment decisions. Stable if
    only AI-generated fields change.

    Also includes PROMPT_VERSION so a prompt bump forces all properties to
    re-enrich even if their scraped data hasn't changed.
    """
    sig_data = {k: getattr(prop, k, None) for k in sorted(CORE_SCRAPED_FIELDS)}
    sig_data["__prompt_version__"] = PROMPT_VERSION
    return hashlib.md5(
        json.dumps(sig_data, sort_keys=True, default=str).encode()
    ).hexdigest()[:12]


def _get_enriched_sig(prop) -> str | None:
    """Read stored enrichment signature from inferred_features."""
    try:
        features = json.loads(prop.inferred_features or "[]")
        for f in features:
            if isinstance(f, str) and f.startswith("enriched_sig:"):
                return f[len("enriched_sig:"):]
    except Exception:
        pass
    return None


def _set_enriched_sig(prop, sig: str) -> None:
    """
    Write the enrichment signature into inferred_features (in-memory, before
    repo.save). Replaces any previously stored signature.
    """
    try:
        existing = json.loads(prop.inferred_features or "[]")
    except Exception:
        existing = []
    existing = [f for f in existing if not (isinstance(f, str) and f.startswith("enriched_sig:"))]
    existing.append(f"enriched_sig:{sig}")
    prop.inferred_features = json.dumps(existing)
...
def enrich_property(prop_id: str, repo) -> None:
    ...
    try:
        ...
        # ── Phase 3C: Skip if scraped inputs haven't changed since last enrichment ──
        ...
        if changed:
            ...
            _set_enriched_sig(prop, current_sig)
            repo.save(prop)
            repo.add_all_logs(log_rows)
            logger.info(
                "ai_enricher: completed %d task(s) for %s (sig %s)",
                len(tasks), prop_id, current_sig,
            )
        else:
            # Nothing AI-changed, but stamp the sig anyway so we don't retry
            # a no-op enrichment endlessly on the same unchanged data.
            _set_enriched_sig(prop, current_sig)
            repo.save(prop)
            logger.info(
                "ai_enricher: no tasks needed for %s, sig %s stored", prop_id, current_sig
            )
